...
export class EventBus {
  ...
  fireEvent(eventName: GmEventName, payload: GmSystemEvent) {
    ...
    this.pendingForward = this.pendingForward
      .catch((error: unknown) => {
        log.error('EventsBus: previous event forwarding failed', error);
      })
      .then(async () => {
        try {
          await this.forwarder.processEvent(eventName, payload);
        } catch (error) {
          log.error('EventsBus: event forwarding failed', eventName, error);
        }
      });
  }
