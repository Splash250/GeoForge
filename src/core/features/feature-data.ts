...
function isCustomPropertyName(name: string): boolean {
  ...
  return !typedKeys(propertyValidators).some(
    (propertyName) => name === propertyName || name === `gm_${propertyName}`,
  );
}
...
export class FeatureData {
  ...
  parseExtraProperties(geoJson: GeoJsonShapeFeature) {
    return Object.fromEntries(
      Object.entries(cloneDeep(geoJson.properties) || {}).filter(([name]) =>
  ...
  getProperty<TValue = unknown>(name: string): TValue | undefined {
    if (!isCustomPropertyName(name)) {
      return undefined;
    }

    const value = this.getGeoJson().properties[name];
  ...
  >(): Partial<TProperties> {
    return cloneDeep(this.parseExtraProperties(this.getGeoJson())) as Partial<TProperties>;
  }
  ...
  updateGeometry(geometry: BasicGeometry) {
    ...
    const before = cloneDeep(featureGeoJson);
  ...
  /**
   * Restores a previously captured full GeoJSON snapshot.
   *
   * @internal
   */
  restoreGeoJsonSnapshot(snapshot: GeoJsonShapeFeature): void {
    const currentProperties = this.getGeoJson().properties;
    const restoredGeoJson = cloneDeep(snapshot);
    const rollbackDiffProperties: Record<string, unknown> = { ...restoredGeoJson.properties };

    for (const key of Object.keys(currentProperties)) {
      if (!(key in restoredGeoJson.properties)) {
        rollbackDiffProperties[key] = undefined;
      }
    }

    this._geoJson = restoredGeoJson;
    const rollbackDiffGeoJson = cloneDeep({
      ...restoredGeoJson,
      properties: rollbackDiffProperties,
  ...
  updateProperties(properties: Record<string, unknown>) {
    ...
    const before = cloneDeep(this._geoJson);
    ...
    const customPropertyUpdates = Object.fromEntries(
      Object.entries(properties).filter(([key]) => isCustomPropertyName(key)),
    );
    ...
    for (const [key, value] of Object.entries(this._geoJson.properties)) {
      if (key in customPropertyUpdates) {
    ...
    for (const [key, value] of Object.entries(customPropertyUpdates)) {
  ...
  setProperties(properties: Record<string, unknown>) {
    ...
    for (const [key, value] of Object.entries(properties)) {
      if (value !== undefined && isCustomPropertyName(key) && !(key in mandatoryProperties)) {
  ...
  fireFeatureUpdatedEvent({ mode }: { mode: EditModeName }) {
    const sourceGeoJson = cloneDeep(this.getGeoJson());
  ...
  private recordUpdate(
    ...
    this.gm.history?.record(
      [
        {
          kind: 'update',
          ref: { sourceName: this.sourceName, featureId: this.id },
          before,
