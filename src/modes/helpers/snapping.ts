...
export class SnappingHelper extends BaseHelper {
  ...
  removeExcludedFeature(featureData: FeatureData) {
    this.excludedFeature.delete(featureData);
  }
  ...
  getSnappedLngLat(lngLat: LngLatTuple, point: ScreenPoint): LngLatTuple {
    ...
    snappingLngLat = this.getLineEndpointSnapping(features, point);
    if (snappingLngLat) {
      return snappingLngLat;
    }
  ...
  getLineEndpointSnapping(features: Array<FeatureData>, point: ScreenPoint): LngLatTuple | null {
    if (!this.lineEndpointSnappingOptions.enabled) {
      return null;
    }

    const endpoint = this.gm.geometry.getNearestLineEndpoint(features, point, {
      maxPixelDistance: this.lineEndpointSnappingOptions.maxPixelDistance ?? this.tolerance,
      endpoints: this.lineEndpointSnappingOptions.endpoints,
      excludeFeatures: this.lineEndpointSnappingOptions.excludeFeatures,
    });

    return endpoint ? endpoint.coordinate : null;
  }
    ...
    return (
      this.gm.features
        .getFeaturesByScreenBounds({
          bounds,
          sourceNames: this.lineEndpointSnappingOptions.sourceNames ?? [
            SOURCES.main,
            SOURCES.temporary,
          ],
        })
