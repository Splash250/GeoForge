<script lang="ts">
  ...
  $effect(() => {
    ...
    const eventHandlers = events.map((eventName) => {
      const handler = (e: { type: string; feature?: { id?: string }; action?: string }) => {
        const eventData = {
          type: e.type,
    ...
    const mouseMoveHandler = (e: { lngLat: { lng: number; lat: number } }) => {
    ...
    const layerInfoIntervalId = window.setInterval(updateLayerInfo, 1000);
    ...
    return () => {
      ...
      window.clearInterval(layerInfoIntervalId);
    };
  });
