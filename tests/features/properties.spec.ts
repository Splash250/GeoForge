...
test.describe('Feature Properties Management', () => {
  ...
  test.describe('property helper methods', () => {
    test('reads, updates, deletes, and clones custom properties', async () => {
      const markerFeature: GeoJsonImportFeature = {
        id: 'property-helper-test-1',
        type: 'Feature',
        properties: {
          shape: 'marker',
          segmentValue: 300,
      ...
      const result = await page.evaluate(() => {
        const feature = window.geoman.features.get('gm_main', 'property-helper-test-1');
        if (!feature) return null;

        const nested = feature.getProperty<{ label: string }>('nested');
        if (nested) {
          nested.label = 'Mutated outside';
        }

        feature.updateProperty('segmentValue', 450);
        feature.updateProperty('temporary', 'remove me');
        feature.updateProperty('temporary', undefined);

        const typedProperties = feature.getTypedProperties<{
          segmentValue: number;
          nested: { label: string };
        }>();
        if (typedProperties.nested) {
          typedProperties.nested.label = 'Mutated typed properties outside';
        }

        return {
          segmentValue: feature.getProperty<number>('segmentValue'),
          nested: feature.getProperty<{ label: string }>('nested'),
          temporary: feature.getProperty('temporary'),
          properties: feature.getTypedProperties<{
            segmentValue: number;
            nested: { label: string };
          }>(),
          shape: feature.shape,
    ...
    test('does not expose Geoman system properties through typed custom properties', async () => {
      const markerFeature: GeoJsonImportFeature = {
        id: 'property-helper-test-2',
        type: 'Feature',
        properties: {
          shape: 'marker',
          gm_id: 'legacy-system-id',
          gm_shape: 'polygon',
          name: 'Visible name',
          _gmCustom: 'kept custom value',
          _gml_id: 'kept app id',
          '_gm:internal': 'hidden',
      ...
      const result = await page.evaluate(() => {
        const feature = window.geoman.features.get('gm_main', 'property-helper-test-2');
        if (!feature) return null;

        return {
          gmId: feature.getProperty('gm_id'),
          gmShape: feature.getProperty('gm_shape'),
          name: feature.getProperty('name'),
          shape: feature.getProperty('shape'),
          systemInternal: feature.getProperty('_gm:internal'),
          prefixedShape: feature.getProperty('__gm_shape'),
          properties: feature.getTypedProperties<{
            name: string;
            gm_id?: string;
            gm_shape?: string;
      ...
      expect(result).toEqual({
        ...
        name: 'Visible name',
        shape: undefined,
        systemInternal: undefined,
        prefixedShape: undefined,
        properties: {
          name: 'Visible name',
          _gmCustom: 'kept custom value',
          _gml_id: 'kept app id',
    ...
    test('does not write Geoman system properties through application property APIs', async () => {
      const markerFeature: GeoJsonImportFeature = {
        id: 'property-helper-test-3',
        type: 'Feature',
        properties: {
          shape: 'marker',
          name: 'Visible name',
      ...
      const result = await page.evaluate(() => {
        const feature = window.geoman.features.get('gm_main', 'property-helper-test-3');
        if (!feature) return null;

        feature.updateProperty('shape', 'polygon');
        feature.updateProperty('gm_shape', 'polygon');
        ...
        const transaction = window.geoman.transactions.start({ id: 'system-property-write-test' });
        transaction.updateProperty(feature, 'gm_id', 'hacked-id');
        ...
        return {
          ...
          shape: feature.shape,
          properties: feature.getGeoJson().properties,
          typedProperties: feature.getTypedProperties<{
            segmentValue: number;
            label: string;
            shape?: string;
            gm_id?: string;
      ...
      expect(result!.segmentValue).toBe(450);
      expect(result!.label).toBe('Line A');
      expect(result!.shape).toBe('marker');
      expect(result!.typedProperties).toEqual({
        segmentValue: 450,
        label: 'Line A',
        name: 'Visible name',
      });
      expect(result!.properties.shape).toBeUndefined();
      expect(result!.properties.gm_id).toBeUndefined();
      expect(result!.properties.gm_shape).toBeUndefined();
      expect(result!.properties['_gm:internal']).toBeUndefined();
      expect(result!.properties.__gm_shape).toBe('marker');
    });
  });
