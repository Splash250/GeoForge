...
let map: Map;
let geoman: InstanceType<typeof GeoForge>;
...
export function startLegacyPlayground(app: HTMLDivElement) {
  resetLegacyState();

  const mountId = ++activeLegacyMountId;
  let disposed = false;
  const isCurrentMount = () => !disposed && isActiveLegacyMount(mountId);
...
`;
  ...
  controls = requireElement<HTMLElement>('#controls');
  status = requireElement<HTMLElement>('#status');
  navButtons = Array.from(document.querySelectorAll<HTMLButtonElement>('.nav-button'));
  ...
  activeMap.addControl(new maplibregl.NavigationControl({ visualizePitch: true }), 'bottom-left');

  geoman = new GeoForge(activeMap, {
    layerStyles: {
      line: {
  ...
  geoman.tools.register({
    ...
    onStart: ({ geoman }) => {
      ...
      selectedLineId = geoman.getSelectedFeature()?.id
        ...
        : selectedLineId;
      status.textContent =
        'Interaction mode active. Hover a line, then click it to edit that line.';
  ...
  activeMap.on('load', async () => {
    await ensureBaseSymbolImages(activeMap, mountId);
    if (!isCurrentMount()) {
      return;
    }

    activeMap.addSource('lab-lines', {
      type: 'geojson',
      data: {
  ...
  activeMap.on('gm:select', (event: SelectFwdEvent) => {
    if (!isCurrentMount()) {
      return;
    }
  ...
  return () => {
    ...
    try {
      ...
      // MapLibre can throw if cleanup races with initialization.
    }
...
function resetLegacyState() {
  decoratorFormState = { ...baseFormState };
...
function isActiveLegacyMount(mountId: number) {
  return mountId === activeLegacyMountId;
}
...
function isLegacyHash(hash: string) {
  return hash === '' || hash === '#decorators' || hash === '#overlays';
}
...
function wireDecoratorControls() {
  ...
  controls.querySelector('#add-decorator')?.addEventListener('click', async () => {
    const mountId = activeLegacyMountId;
    readDecoratorForm();
    const decorator = await buildDecoratorFromForm(mountId);

    if (!isActiveLegacyMount(mountId) || !decorator) {
      return;
    }
...
function wireSvgAuthoringControls() {
  const renderMountId = activeLegacyMountId;
  ...
  svgInput.addEventListener('paste', () =>
    window.setTimeout(() => {
      if (!isActiveLegacyMount(renderMountId)) {
        return;
      }
...
async function buildDecoratorFromForm(mountId: number): Promise<LineDecoratorOptions | null> {
  ...
  const imageId = await resolveSymbolImageId(mountId);
  if (!imageId) {
    return null;
  }
...
async function syncDecorators() {
  const mountId = activeLegacyMountId;
  const runId = ++syncRun;
  await ensureImagesForDecorators(mountId);
  if (!isActiveLegacyMount(mountId) || runId !== syncRun) {
    return;
  }
...
async function ensureBaseSymbolImages(targetMap: Map, mountId: number) {
  await Promise.all([
    ensureSvgImage(targetMap, 'lab-chevron', chevronSvg('#0f766e'), mountId),
    ensureSvgImage(targetMap, 'lab-diamond', diamondSvg('#be123c'), mountId),
    ensureSvgImage(targetMap, 'lab-dot', dotSvg('#2563eb'), mountId),
  ]);
}

async function ensureImagesForDecorators(mountId: number) {
  if (!isActiveLegacyMount(mountId)) {
    return;
  }
  ...
  await Promise.all(
    symbols.map((decorator) => {
      ...
      if (targetMap.hasImage(decorator.imageId)) {
        return Promise.resolve();
      }

      return ensureSvgImage(
        targetMap,
        decorator.imageId,
        chevronSvg(decorator.color ?? '#0f766e'),
        mountId,
      );
    }),
  );
...
async function resolveSymbolImageId(mountId: number) {
  if (decoratorFormState.symbolPreset === 'custom') {
    const id = `custom-svg-${Date.now().toString(36)}`;
    const imageReady = await ensureSvgImage(
      map,
      id,
      mergeSvgCss(decoratorFormState.customSvg, decoratorFormState.customSvgCss),
      mountId,
    );
    return imageReady ? id : null;
  }
...
async function ensureSvgImage(targetMap: Map, id: string, svg: string, mountId: number) {
  if (!isActiveLegacyMount(mountId)) {
    return false;
  }
