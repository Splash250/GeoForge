...
- Race condition in `waitForBaseMap` causing initialization timeout (#119)
...
- Empty toolbar groups now hidden and marker temporary styles are respected (#117)
...
- Run prettier formatter on codebase (#118)

## [0.6.0] - 2025-12-19

### Added

- `overwrite` option for `importGeoJson()` to replace existing features with the same ID (#113)
...
- New simplified feature data API for `FeatureData` (#77)
...
- Property deletion now works correctly without causing MapLibre encoding errors (#77)
...
- Dev panel "Clear All Shapes" now properly clears the internal featureStore (#112)
...
- Circle features now appear in `source.getGeoJson()` immediately after creation (#110)
...
- Drag events (`gm:dragstart`, `gm:drag`, `gm:dragend`) now fire in correct order to user event listeners (#110)
...
- Improved `waitForPendingUpdates` reliability for edge cases where MapLibre's internal `_isUpdatingWorker` guard caused early promise resolution (#110)

### Changed

- `exportGeoJson()` now uses internal `FeatureData` state (`getGmGeoJson()`) instead of MapLibre's `serialize()`, ensuring newly created features are always included even during event handlers (#110)

## [0.5.8] - 2025-12-17

### Added

- Interactive dev panels with settings, layer monitoring, and debugging tools (#102)
- Advanced testing for draw and edit modes, including CRUD operations and GeoJSON utilities (#95)
- Prepare script for git installs

### Fixed

- Improved destroy/reinit cleanup for React StrictMode compatibility (#101)
- Test stability after dev panels were added (#105)

## [0.5.7] - 2025-12-16

### Added

- Multi-LineString support (#89)
- New `awaitDataUpdatesOnEvents` setting to control event timing behavior (#99)
...
- Rectangle drawing initial geometry (#96)
- Event ordering issues by leveraging MapLibre's `updateData` with `waitForCompletion` (#99). Events like `dragstart`, `drag`, and `dragend` now fire in the correct sequence

## [0.5.6] - 2025-12-12

### Fixed

- Replace `target` property name to `mode` in GmControlEvent (#92)
...
- Simplify delayed source update (#86)
...
- Disable edit on a per-feature basis (#45)
- `idPropertyName` option for GeoJSON export (#71)
- Optional `idGenerator` for feature ID creation (#75)
- Geoman event declarations to MapLibre types (#74)
- `deleteAll` method (#79)
