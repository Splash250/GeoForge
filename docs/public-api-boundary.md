...
- Prefer `geoForge.overlays.html.upsert(definition)` when state synchronization provides a complete HTML overlay definition. `geoForge.overlays.html.add(definition)` is retained as a backward-compatible alias for the same upsert behavior; use `update(id, patch)` for partial changes.
